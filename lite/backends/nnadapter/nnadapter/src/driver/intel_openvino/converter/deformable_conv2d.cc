...
int ConvertDeformableConv2d(Converter* converter, core::Operation* operation) {
  ...
  auto ov_pads_end =
      ov::CoordinateDiff({static_cast<std::ptrdiff_t>(pads[1]),
                          static_cast<std::ptrdiff_t>(pads[3])});
  auto auto_pad = ov::op::PadType::EXPLICIT;
  auto deformable_conv2d_op = std::make_shared<ov::opset8::DeformableConvolution>(
      *input_tensor,
      *offset_tensor,
      *filter_tensor,
      *mask_tensor,
  ...
  auto add_op = std::make_shared<default_opset::Add>(deformable_conv2d_op,
