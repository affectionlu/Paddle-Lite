...
int ConvertReduce(Converter* converter, core::Operation* operation) {
  ...
  auto axes_tensor = converter->AddConstantTensor(std::vector<int32_t>(
      axes_data, axes_data + axes_size));
  switch (operation->type) {
#define CONVERT_REDUCE(type, reduce_type)                          \
  case NNADAPTER_##type: {                                         \
    auto reduce_op = std::make_shared<reduce_type>(                \
        *input_tensor, *axes_tensor, keep_dim);                    \
