...
int ConvertCumSum(Converter* converter, core::Operation* operation) {
  ...
  auto axis_tenor = converter->AddConstantTensor<int32_t>(axis);
  auto cumsum_op = std::make_shared<default_opset::CumSum>(*input_tensor, *axis_tensor, exclusive, reverse);
