...
int ConvertLayerNormalization(Converter* converter,
  ...
  auto normalized_shape_tensor = converter->AddConstantTensor(normalized_shape);
  // Layer normalization
  auto mvn_op = std::make_shared<default_opset::MVN>(*input_tensor, *normalized_shape_tensor, true, epsilon, ov::op::MVNEpsMode::INSIDE_SQRT);
  auto multiply_op = std::make_shared<default_opset::Multiply>(mvn_op->output(0), *scale_tensor);
  auto add_op = std::make_shared<default_opset::Add>(multiply_op->output(0), *bias_tensor);
